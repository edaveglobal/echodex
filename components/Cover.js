...
const MyComponent = () => {
    ...
    async function buyToken(amount) {
        ...
        // Ensure address and contractAddress are defined
        const address = await ethereum.request({ method: 'eth_accounts' }).then(accounts => accounts[0]);
        const contractAddress = "0x62AD35EbBBC4BE1cC025Da4265D0731cA933B2CC";  // Replace with your contract address
    
        // Ensure the user is connected
        if (!address) {
            console.error("User is not connected to MetaMask.");
            return;
        }
    
        // Convert amount to Wei
        let _price = web3.utils.toWei(amount, "ether");  // Specify the correct unit (ether or gwei)
    
        // Prepare the transaction
        let tx = {
            from: address,
            to: contractAddress,
            value: web3.utils.numberToHex(_price)  // Convert price to hex format
        };
    ...
    bscscanTokenUrl_ = "https://basescan.org/token/" + tokenData.token;
    bscscanContractUrl_ = "https://basescan.org/address/" + tokenData.presaleContract;
    tokenImageUrl_ = tokenData.tokenIconURL;
    let audited_ = tokenData.smartContractAudit;
    let verified_ = tokenData.kycVerified;

    var zeros = ""
    for (var i = 0; i < tokenData.totalSupplyNumberOfZeros; i++) {
        zeros += "0"
    }
    let totalSupply_ = tokenData.totalSupplyInitial + zeros;

    zeros = ""
    for (var j = 0; j < tokenData.tokenForPresaleNumberOfZeros; j++) {
        zeros += "0"
    }
    let tokenForPresale_ = tokenData.tokenForPresaleInitial + zeros;
